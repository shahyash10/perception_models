...
def compute_metrics(jsonl_file, csv_file, extra_outdir=None):
    ...
    file_path = f"./answers/_mmvet_submission_{match.group(1)}.json"
    ...
    print(f"submission file: {file_path} is created, please submit at [https://huggingface.co/spaces/whyu/MM-Vet_Evaluator]\n and update the {csv_file} sheet")
    with open("./mmvet_submission_url.txt", "w") as f:
        f.write("https://huggingface.co/spaces/whyu/MM-Vet_Evaluator")

    # add a duplicate copy of the info to the extra_outdir
    if extra_outdir is not None:
        os.makedirs(extra_outdir, exist_ok=True)
        extra_submission_file = os.path.join(extra_outdir, f"mmvet_submission_{model}.json")
        with open(extra_submission_file, "w") as json_file:
            json.dump(test_list, json_file)
        with open(os.path.join(extra_outdir, "mmvet_submission_url.txt"), "w") as f:
            f.write("https://huggingface.co/spaces/whyu/MM-Vet_Evaluator")

        print(f"Added a copy of the submission file to {extra_submission_file}")

        extra_csv_file = os.path.join(extra_outdir, f"mmvet_{model}.csv")
